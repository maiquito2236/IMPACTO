...
class ReporteModel {
    ...
    // 🛡️ Obtener el ID real del odontólogo a partir del usuario en sesión
    public function obtenerIdOdontologo($idUsuario) {
        $sql = "SELECT ID_ODONTOLOGO FROM odontologo WHERE USUARIOS_ID_USUARIOS = ?";
        $stmt = $this->db->prepare($sql);
        $stmt->execute([$idUsuario]);
        $res = $stmt->fetch(PDO::FETCH_ASSOC);
        return $res ? $res['ID_ODONTOLOGO'] : null;
    }
    ...
    // 👤 3. Contar pacientes tratados (que tienen historia clínica con este doctor)
    public function contarPacientesActivos($idOdontologo) {
        $sql = "SELECT COUNT(DISTINCT hc.PACIENTE_ID_PACIENTE) AS total 
                ...
                WHERE hc.ODONTOLOGO_ID_ODONTOLOGO = ? AND u.ESTADO = 'Activo'";
        $stmt = $this->db->prepare($sql);
    ...
    // 📊 4. Contar tratamientos activos (Historias clínicas totales del doctor)
    public function contarTratamientosActivos($idOdontologo) {
 ...
 // 🏥 6. Obtener pacientes recientes (con detalles para el modal de tu vista)
    public function obtenerPacientesRecientes($idOdontologo) {
        // Agregamos u.NUMERO_DOCUMENTO a la selección
        $sql = "SELECT p.ID_PACIENTE, u.NUMERO_DOCUMENTO, CONCAT(u.NOMBRES, ' ', u.APELLIDOS) AS paciente_nombre, u.TELEFONO, 
                ...
                ORDER BY MAX(hc.FECHA_REGISTRO) DESC LIMIT 5";
        $stmt = $this->db->prepare($sql);
        $stmt->execute([$idOdontologo]);
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    }

    // 💰 7. Ingresos por rango de fechas (Cruce completo: pago -> factura -> hc -> odontologo)
    public function obtenerIngresosPorRango($inicio, $fin, $idOdontologo) {
        $sql = "SELECT IFNULL(SUM(pg.MONTO), 0) AS total 
                ...
                AND UPPER(pg.ESTADO) = 'PAGADO' 
                AND hc.ODONTOLOGO_ID_ODONTOLOGO = ?";
        $stmt = $this->db->prepare($sql);
        $stmt->execute([$inicio, $fin, $idOdontologo]);
        $res = $stmt->fetch(PDO::FETCH_ASSOC);
        return $res ? (float)$res['total'] : 0.0;
    }

    // 🍩 8. Obtener la distribución real de tratamientos para el gráfico de dona
    public function obtenerDistribucionTratamientos($idOdontologo) {
        // Obtenemos el total de procedimientos de este doctor para calcular los porcentajes
        ...
        $total = ($resTotal && $resTotal['total'] > 0) ? (int)$resTotal['total'] : 1;

        // Obtenemos el desglose
        $sql = "SELECT p.NOMBRE_PROCEDIMIENTO AS nombre, COUNT(hcp.PROCEDIMIENTOS_ID_PROCEDIMIENTO) AS cantidad
                FROM historia_clinica_has_procedimientos hcp
        ...
        foreach ($resultados as $row) {
            $datosGrafico[] = [
                ...
                'porcentaje' => round(($row['cantidad'] / $total) * 100)
            ];
        }
        return $datosGrafico;
    }
